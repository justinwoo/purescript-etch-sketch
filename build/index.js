...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var screenWipes = $Signal.mailbox(true);
   var keyboard = Elm.Native.Port.make(_elm).inboundSignal("keyboard",
   "Char.KeyCode",
   function (v) {
      return typeof v === "number" && isFinite(v) && Math.floor(v) === v ? v : _U.badPort("an integer",
      v);
   });
   var point = F3(function (increment,subkey,_p0) {
      var _p1 = _p0;
      var _p3 = _p1._1;
      var _p2 = _p1._0;
      return A2($Svg.rect,
      _U.list([$Html$Attributes.key(A2($Basics._op["++"],
              subkey,
              A2($Basics._op["++"],
              $Basics.toString(_p2),
              A2($Basics._op["++"],",",$Basics.toString(_p3)))))
              ,$Svg$Attributes.width($Basics.toString(increment))
              ,$Svg$Attributes.height($Basics.toString(increment))
              ,$Svg$Attributes.x($Basics.toString(_p2 * increment))
              ,$Svg$Attributes.y($Basics.toString(_p3 * increment))]),
      _U.list([]));
   });
   var view = function (state) {
      var point$ = point(state.increment);
      var cursor = A2(point$,"cursor",state.cursor);
      var points = A2($List.map,point$("point"),state.points);
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,
              _U.list([]),
              _U.list([A2($Html.button,
              _U.list([A2($Html$Events.onClick,screenWipes.address,true)]),
              ...
              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                       ,_0: "border"
                                                       ,_1: "1px solid black"}]))
                      ,$Svg$Attributes.width($Basics.toString(state.width))
                      ,$Svg$Attributes.height($Basics.toString(state.height))]),
              A2($List._op["::"],cursor,points))]))]));
   };
   var wipeScreen = F2(function (_p4,state) {
      return _U.update(state,{points: _U.list([])});
   });
   var insertPoint = F2(function (point,points) {
      var _p5 = A2($List.member,point,points);
      if (_p5 === true) {
            return points;
         } else {
            return A2($List._op["::"],point,points);
         }
   });
   var isInvalidPoint = F2(function (state,coords) {
      var _p6 = coords;
      var _p8 = _p6._1;
      var _p7 = _p6._0;
      return _U.cmp(_p7,0) < 0 ? true : _U.cmp(_p8,
      0) < 0 ? true : _U.cmp(state.increment * (_p7 + 1),
      state.width) > 0 ? true : _U.cmp(state.increment * (_p8 + 1),
      state.height) > 0 ? true : false;
   });
   var State = F5(function (a,b,c,d,e) {
      return {cursor: a,points: b,width: c,height: d,increment: e};
   });
   var Coords = F2(function (a,b) {
      return {ctor: "Coords",_0: a,_1: b};
   });
   var initState = {cursor: A2(Coords,0,0)
                   ,points: _U.list([])
                   ,width: 800
                   ,height: 600
                   ,increment: 10};
   var moveCursor = F2(function (direction,state) {
      var _p9 = state.cursor;
      var _p12 = _p9._1;
      var _p11 = _p9._0;
      var cursor$ = function () {
         var _p10 = direction;
   ...
   var projects = $Signal.mergeMany(_U.list([A2($Signal.map,
                                            moveCursor,
                                            keyDirections)
                                            ,A2($Signal.map,wipeScreen,screenWipes.signal)]));
   ...
   return _elm.Main.values = {_op: _op
                             ...
                             ,point: point
                             ,view: view
                             ,getKeyDirection: getKeyDirection
                             ,keyDirections: keyDirections
                             ,screenWipes: screenWipes
                             ,projects: projects
                             ,main: main};
};
